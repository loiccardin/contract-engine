...
| Google Drive API v3 | Upload DOCX natif (promesses + contrats), upload PDF, archivage, download | `GOOGLE_SERVICE_ACCOUNT_KEY`, `GOOGLE_DRIVE_ROOT_FOLDER_ID` (promesses), `GOOGLE_DRIVE_ARCHIVE_FOLDER_ID` (archives promesses), `GOOGLE_DRIVE_CONTRATS_ROOT_FOLDER_ID` (racine contrats), `GOOGLE_DRIVE_CONTRATS_ARCHIVE_FOLDER_ID` (archives contrats) | developers.google.com/drive |
